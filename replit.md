...
- **API Design**: A comprehensive RESTful API is provided, covering CRUD operations for clients, services, and appointments. All filter operations use query parameters (e.g., `/api/appointments?clientId=xxx&serviceId=yyy&status=scheduled`) instead of path parameters for better N8N integration compatibility. Single resource lookups also support query parameters (e.g., `/api/appointments?id=apt-123`) for easier N8N configuration. Supports filtering by status (scheduled/completed), client, service, date range, and combinations thereof.
